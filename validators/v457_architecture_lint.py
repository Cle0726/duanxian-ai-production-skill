#!/usr/bin/env python3
"""V4.5.10 execution-spine architecture audit (retains the stable filename)."""
...
def main():
    ...
    for name,obj in [('route_registry',rr),('workflow',wf),('authority_registry',ar),('module_manifest',mm)]:
        if obj.get('skill_version') not in {'4.5.10','4.5.11'}: errors.append(f'{name}: skill_version must be 4.5.10 or 4.5.11')
    ...
    checks={
      ...
      'templates/video_conditioning_asset_architecture.md':['V4.5.7','Scene Color'],
      'templates/color_script_derivation_engine.md':['V4.5.7','自动派生'],
    }
